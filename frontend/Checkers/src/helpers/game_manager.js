...
export default {
	...
	init(options = {}) {
		...
		this.player_one        = new Player({id: appInstanceGlobalVar.$PLAYER_ONE});
		this.player_two        = new Player({id: appInstanceGlobalVar.$PLAYER_TWO});
	...
	makeMove(piece, x, y, taken_piece){
		...
		if(!piece.isKing() && y === this.getLastRow(this.current_player)){
			piece.type = appInstanceGlobalVar.$PIECE_TYPE_KING;
		...
		if(taken_piece){
			...
			const player = (piece.player_id === appInstanceGlobalVar.$PLAYER_ONE) ? this.player_two : this.player_one;
	...
	restore(){
		...
		last_moves.reverse().forEach((move) => {
			...
			if(move.upgraded){
				move.piece.type = appInstanceGlobalVar.$PIECE_TYPE_MAN;
			}
		});
		this.history.player_id = this.current_player.id;
		this.current_player = (this.current_player.id === this.player_one.id) ? this.player_two : this.player_one;
